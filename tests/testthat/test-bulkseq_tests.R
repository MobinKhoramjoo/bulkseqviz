...
test_that("DE analysis, Volcano, Barplot, Log2FC Comp, FC Dotplot, and Gene Boxplot run without error", {
  # Generate synthetic data
  n_genes <- 1000
  n_samples <- 12 # Increased samples for gene boxplot faceting
  cnts <- matrix(as.integer(sample(10:1000, n_genes * n_samples, replace=TRUE)), ncol = n_samples)
  colnames(cnts) <- paste0("S", 1:n_samples)
  rownames(cnts) <- paste0("Gene_", 1:n_genes)

  # 3 groups x 2 regions
  meta <- data.frame(
    condition = rep(c("Ctrl", "TrtA", "TrtB"), each = 4),
    region = rep(c("R1", "R2"), times = 6)
  )
  rownames(meta) <- colnames(cnts)
  ...
  # Run Gene Boxplot (BioMart disabled for test speed/stability)
  p_genebox <- plot_gene_boxplot(obj, genes = target_genes[1:2], x_var = "condition", region_var = "region", biomart_dataset = NULL)
  expect_s3_class(p_genebox, "ggplot")
})
