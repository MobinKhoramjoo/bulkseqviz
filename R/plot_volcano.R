...
#'
#' Creates a publication-ready volcano plot from differential expression results stored
#' in the bulkseq object.
#'
#' @param bs_obj A \code{bulkseq} object containing DE results from \code{DEG()}.
#' @param comparison_id Character string. The name of the comparison to plot (e.g., "Treated_vs_Control").
#'   Must match a name in \code{bs_obj$DE_results}.
#' @param fc_thresh Numeric. Log2 Fold Change threshold for significance coloring. Default 1.
...
#' @export
#' @import ggplot2
#' @importFrom dplyr select mutate arrange case_when if_else
...
#' @examples
#' \dontrun{
#'   plot_volcano(bs_obj, comparison_id = "Treated_vs_Control")
#'   plot_volcano(bs_obj, comparison_id = "Treated_vs_Control", top_labels = 10)
#' }
plot_volcano <- function(bs_obj,
  ...
  p <- ggplot2::ggplot(df, ggplot2::aes(x = .data$log2FoldChange, y = .data$log10p)) +
    ...
    ggplot2::geom_vline(xintercept = c(-fc_thresh, fc_thresh), linetype = "dashed", linewidth = 0.5) +
